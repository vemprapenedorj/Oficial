...
export function FeaturedCard(props: { item: DetailItem, onClick: () => void }) {
  ...
  return (
    <motion.div
      whileHover={{ y: -8 }}
      className={`group relative h-full flex flex-col bg-white rounded-[2.5rem] overflow-hidden transition-all duration-500 ${item.isPremium
    ...
    >
      ...
      <div className="p-5 flex-grow flex flex-col justify-between bg-white cursor-pointer" onClick={onClick}>
        <p className="mb-4 h-[5.25rem] line-clamp-3 text-lg leading-7 text-gray-600">
          {item.description}
        </p>
