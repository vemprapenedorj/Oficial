...
const benefits = [
  { icon: LayoutPanelTop, title: 'Posicionamento em destaque', description: 'Sua marca pode aparecer em áreas estratégicas do portal e nas categorias mais relevantes para o turista.' },
  { icon: Sparkles, title: 'Página exclusiva', description: 'Apresente seu negócio com fotos, informações, descrição personalizada e uma identidade alinhada à sua marca.' },
  { icon: MessageCircle, title: 'Conexão direta', description: 'Direcione o visitante para WhatsApp, site oficial, reservas e redes sociais.' },
  { icon: Compass, title: 'Conteúdo que valoriza', description: 'Mostre sua experiência para pessoas que estão planejando visitar Penedo e a Serra da Mantiqueira.' },
];
  ...
  return (
    <div className="bg-[#f8fbf8] text-penedo-graphite">
      <section className="relative isolate overflow-hidden bg-penedo-forest pt-32 pb-16 sm:pt-40 sm:pb-24 lg:pt-48 lg:pb-32">
        <img src="/assets/imagens/penedo_sightseeing.jpg" alt="Vista de Penedo, da Cachoeira de Deus e da Serra da Mantiqueira" className="absolute inset-0 -z-20 h-full w-full object-cover" fetchPriority="high" decoding="async" />
        ...
        <div className="mx-auto grid max-w-7xl items-end gap-12 px-4 sm:px-6 lg:grid-cols-[1.1fr_.9fr] lg:px-8">
          <div className="max-w-3xl">
            <p className="mb-5 inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-4 py-2 text-xs font-bold uppercase tracking-[0.18em] text-white backdrop-blur-sm"><CircleDotDashed size={15} aria-hidden="true" /> Parcerias Vem Pra Penedo</p>
          ...
          <div className="hidden justify-end lg:flex" aria-hidden="true"><div className="w-64 rounded-[2rem] border border-white/20 bg-white/10 p-6 text-white shadow-2xl backdrop-blur-md"><div className="mb-10 flex h-12 w-12 items-center justify-center rounded-2xl bg-penedo-gold text-penedo-forest"><Sparkles size={24} /></div><p className="text-sm font-bold uppercase tracking-[0.15em] text-penedo-gold">Feito para a região</p><p className="mt-3 text-xl font-bold leading-snug">Uma vitrine acolhedora para experiências que fazem Penedo especial.</p></div></div>
        </div>
      </section>

      <section className="px-4 py-16 sm:px-6 lg:px-8 lg:py-24"><div className="mx-auto max-w-7xl"><div className="max-w-2xl"><p className="text-sm font-bold uppercase tracking-[0.16em] text-penedo-emerald">Parceria que apresenta</p><h2 className="mt-3 text-3xl font-black tracking-tight text-penedo-forest sm:text-4xl">Mais presença para a sua marca</h2></div><div className="mt-10 grid gap-5 sm:grid-cols-2 xl:grid-cols-4">{benefits.map(({ icon: Icon, title, description }) => <article key={title} className="group rounded-[1.75rem] border border-penedo-forest/10 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-xl"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-penedo-mint text-penedo-forest transition group-hover:bg-penedo-forest group-hover:text-white"><Icon size={24} aria-hidden="true" /></div><h3 className="mt-6 text-xl font-bold text-penedo-forest">{title}</h3><p className="mt-3 text-sm leading-relaxed text-slate-600">{description}</p></article>)}</div></div></section>
